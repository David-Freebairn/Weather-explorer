"""
pages/2_Monthly_averages.py — Weather Explorer
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Big picture for rainfall, evaporation and temperature — long-term
monthly averages for the selected station: rainfall and evaporation
(bars/line, left axis, mm) and min/max temperature (lines, right axis,
°C), computed over the full cached record. Underneath, a summary table
of annual rainfall and evaporation totals (min / max / avg across full
years of record).
...
summary = pd.DataFrame({
    "Variable": ["Rainfall", "Evaporation"],
    "Min":  [annual_rain.min(), annual_evap.min()],
    "Max":  [annual_rain.max(), annual_evap.max()],
    "Avg":  [annual_rain.mean(), annual_evap.mean()],
})
...
fig.add_trace(go.Scatter(
    ...
    line=dict(color="rgba(52,152,219,0.95)", width=2.2), mode="lines+markers",
...
fig.update_layout(
    title=dict(
        text=f"Monthly average: rain, evap., min/max temperature, "
...
def _build_jpeg() -> io.BytesIO:
    ...
    fig_m.suptitle(
        f"Monthly average: rain, evap., min/max temperature, {station['name']}, "
